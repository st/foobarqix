...
;; Instead a clojure solution (from an apprentice) written with simplicity in mind. I hope it may teach a little to newbies.(Sources : [https://github.com/st/foobarqix](https://github.com/st/foobarqix))

;; Feedbacks very welcome
;; (@stephane_tavera on twitter)

;; Marginalia is a fantastic tool (you're just reading its output). Code and documentation are "one and the same".
;; Thank you @fogus ! [http://fogus.me/fun/marginalia/](http://fogus.me/fun/marginalia/)
...
;; Let's explain this function from inside to outside.We build a string with x (we are expecting x to be a number) thanks to (str x). We then take the sequence of characters with seq. And we substract 48 to the int value for each element.(48 is the code of '0'.)

(defn to-ints 
	[x]
    (map #(- (int %) 48) (seq (str x))))

;; What better documentation than unit tests 	[here](https://github.com/st/foobarqix/blob/master/test/foobarqix/test/core.clj) ?
...
;; `to-ints` retrieves the list of digits `x`. For each digit we apply and concate the `rules` map (remember it's also a function) thanks to `mapcat`. 

(defn has [[x accu]]
    [x (apply str accu (mapcat rules (to-ints x)))])

;; In case (very likely) where previous documentation was unclear, have a look at unit tests [here](https://github.com/st/foobarqix/blob/master/test/foobarqix/test/core.clj) !
...
;; Unclear ? OK,you now know the way to at unit tests : [here](https://github.com/st/foobarqix/blob/master/test/foobarqix/test/core.clj)
